...
- Светлая/тёмная тема — SSR-корректна через cookie (читается на сервере в `app/layout.tsx`), без мигания дефолтом при загрузке; черновик настроек автосохраняется в `localStorage`
